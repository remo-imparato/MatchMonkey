...
//localRequirejs('SimilarArtistsTools');
...
let saveState = function (onlyMode) {
    ...
    SimilarArtists.scrobblerState = scrobblerState;
}

optionPanels.pnl_Library.subPanels.pnl_SimilarArtists.load = function (sett) {
    fmUI = getAllUIElements(qid('SimilarArtistsSettings'));
    scrobblerState = SimilarArtists.scrobblerState;
    ...
    let updateUserInfo = function () {
        ...
        localPromise(SimilarArtists.getUserInfo()).then(function (ui) {
    ...
    fmUI.chbScrobblerOn.controlClass.localListen(fmUI.chbScrobblerOn, 'change', function () {
        if (fmUI.chbScrobblerOn.controlClass.checked && !scrobblerState.sessionKey && !authorizing) {
            ...
            SimilarArtists.authorize().then(function (sessionKey) {
